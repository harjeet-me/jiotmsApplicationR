...
export const ACTION_TYPES = {
  SEARCH_BOOKINGS: 'booking/SEARCH_BOOKINGS',
...
export default (state: BookingState = initialState, action): BookingState => {
  switch (action.type) {
    case REQUEST(ACTION_TYPES.SEARCH_BOOKINGS):
    ...
    case FAILURE(ACTION_TYPES.SEARCH_BOOKINGS):
    ...
    case SUCCESS(ACTION_TYPES.SEARCH_BOOKINGS):
...
const apiSearchUrl = 'api/_search/bookings';

// Actions

export const getSearchEntities: ICrudSearchAction<IBooking> = (query, page, size, sort) => ({
  type: ACTION_TYPES.SEARCH_BOOKINGS,
  payload: axios.get<IBooking>(`${apiSearchUrl}?query=${query}${sort ? `&page=${page}&size=${size}&sort=${sort}` : ''}`)
});
