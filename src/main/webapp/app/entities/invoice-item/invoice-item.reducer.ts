...
export const ACTION_TYPES = {
  SEARCH_INVOICEITEMS: 'invoiceItem/SEARCH_INVOICEITEMS',
...
export default (state: InvoiceItemState = initialState, action): InvoiceItemState => {
  switch (action.type) {
    case REQUEST(ACTION_TYPES.SEARCH_INVOICEITEMS):
    ...
    case FAILURE(ACTION_TYPES.SEARCH_INVOICEITEMS):
    ...
    case SUCCESS(ACTION_TYPES.SEARCH_INVOICEITEMS):
...
const apiSearchUrl = 'api/_search/invoice-items';

// Actions

export const getSearchEntities: ICrudSearchAction<IInvoiceItem> = (query, page, size, sort) => ({
  type: ACTION_TYPES.SEARCH_INVOICEITEMS,
  payload: axios.get<IInvoiceItem>(`${apiSearchUrl}?query=${query}`)
});
