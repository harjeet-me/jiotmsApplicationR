...
export const ACTION_TYPES = {
  SEARCH_EQUIPMENT: 'equipment/SEARCH_EQUIPMENT',
...
export default (state: EquipmentState = initialState, action): EquipmentState => {
  switch (action.type) {
    case REQUEST(ACTION_TYPES.SEARCH_EQUIPMENT):
    ...
    case FAILURE(ACTION_TYPES.SEARCH_EQUIPMENT):
    ...
    case SUCCESS(ACTION_TYPES.SEARCH_EQUIPMENT):
...
const apiSearchUrl = 'api/_search/equipment';

// Actions

export const getSearchEntities: ICrudSearchAction<IEquipment> = (query, page, size, sort) => ({
  type: ACTION_TYPES.SEARCH_EQUIPMENT,
  payload: axios.get<IEquipment>(`${apiSearchUrl}?query=${query}`)
});
