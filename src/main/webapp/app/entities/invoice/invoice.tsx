    ...
    return (
      <div>
        <h2 id="invoice-heading">
          <Translate contentKey="jiotmsApplicationRApp.invoice.home.title">Invoices</Translate>
          <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="jiotmsApplicationRApp.invoice.home.createLabel">Create a new Invoice</Translate>
          </Link>
        </h2>
        <Row>
          <Col sm="12">
              ...
              <AvGroup>
                <InputGroup>
                  <AvInput
                    ...
                    placeholder={translate('jiotmsApplicationRApp.invoice.home.search')}
                  />
        ...
        <div className="table-responsive">
          {invoiceList && invoiceList.length > 0 ? (
              ...
              <tbody>
                {invoiceList.map((invoice, i) => (
                  <tr key={`entity-${i}`}>
                    ...
                    <td>{invoice.bookingNo}</td>
                    <td>{invoice.invoiceTotal}</td>
                    <td>
                      <TextFormat type="date" value={invoice.invoiceDueDate} format={APP_DATE_FORMAT} />
                    </td>
                    <td>
                      <Translate contentKey={`jiotmsApplicationRApp.StatusEnum.${invoice.status}`} />
                    </td>
                    <td>{invoice.invoiceTo ? <Link to={`customer/${invoice.invoiceTo.id}`}>{invoice.invoiceTo.id}</Link> : ''}</td>
            ...
            <div className="alert alert-warning">
              <Translate contentKey="jiotmsApplicationRApp.invoice.home.notFound">No Invoices found</Translate>
            </div>
          )}
        </div>
      </div>
